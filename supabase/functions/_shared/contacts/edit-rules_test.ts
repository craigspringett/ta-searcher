...
const site = [
  { name: 'Mrs H Patel', role: 'Headteacher', email: 'head@oak.sch.uk', confidence: 'found' },
  { name: 'Mr A Brown', role: 'Deputy Headteacher', email: '', confidence: 'role_only' },
];
...
Deno.test('parseContactEditRequest: the shape, the address check, the required name and reason, the key', () => {
  ...
  assertEquals(parseContactEditRequest({ action: 'edit', companySearchId: company, name: 'Mrs H Patel', email: 'hpatel@oak' }).error, 'That does not look like an email address. Check it: name@company.sch.uk.');
  ...
  const e = parsed({ action: 'edit', contactKey: 'HPATEL', name: '  Mrs  H Patel ', role: '', email: ' HPatel@Oak.sch.uk ', phone: '020 7946 0000', note: 'from the office' });
  assertEquals(e, { action: 'edit', companySearchId: company, contactKey: 'hpatel', name: 'Mrs H Patel', role: null, email: 'hpatel@oak.sch.uk', phone: '020 7946 0000', note: 'from the office' });
...
Deno.test('checkContactEdit: adds must be new, edits and removes must name someone listed or removed, one address per person', () => {
  ...
  assertEquals(checkContactEdit(parsed({ action: 'add', name: 'Mr J Khan', email: 'sbm@oak.sch.uk' }), contacts, []), null);
  assertEquals(checkContactEdit(parsed({ action: 'add', name: 'Mr J Khan', email: 'head@oak.sch.uk' }), contacts, []), 'head@oak.sch.uk is already listed for Mrs H Patel.');
  ...
  assertEquals(checkContactEdit(parsed({ action: 'edit', contactKey: 'abrown', name: 'Mr A Brown', email: 'head@oak.sch.uk' }), contacts, []), 'head@oak.sch.uk is already listed for Mrs H Patel.');
  assertEquals(checkContactEdit(parsed({ action: 'edit', contactKey: 'hpatel', name: 'Mrs H Patel', email: 'head@oak.sch.uk' }), contacts, []), null);
...
Deno.test('an edited address moves an active follow-up run to the new address, with a note for the Calls history', () => {
  const runs: ActiveSequence[] = [
    { id: 'd1', status: 'active', contact_name: 'Mrs Patel', contact_email: 'head@oak.sch.uk', contact_role: 'Headteacher' },
    { id: 'd2', status: 'stopped', contact_name: 'Mr A Brown', contact_email: 'deputy@oak.sch.uk', contact_role: null },
  ];
  ...
  const change = sequenceEmailChange(parsed({ action: 'edit', contactKey: 'hpatel', name: 'Mrs H Patel', email: 'hpatel@oak.sch.uk' }), current, runs, 'Anja');
  assertEquals(change, { sequenceId: 'd1', from: 'head@oak.sch.uk', to: 'hpatel@oak.sch.uk', note: 'Email for Mrs Patel changed to hpatel@oak.sch.uk by Anja.' });
  // Matched by key when the names agree.
  const byKey = sequenceEmailChange(parsed({ action: 'edit', contactKey: 'patel', name: 'Mrs Patel', email: 'new@oak.sch.uk' }), null, runs, 'Kim');
  assertEquals(byKey?.sequenceId, 'd1');
  // Nothing to do: the same address, no address, a stopped run, or a different person.
  assertEquals(sequenceEmailChange(parsed({ action: 'edit', contactKey: 'hpatel', name: 'Mrs H Patel', email: 'head@oak.sch.uk' }), current, runs, 'Anja'), null);
  assertEquals(sequenceEmailChange(parsed({ action: 'edit', contactKey: 'hpatel', name: 'Mrs H Patel', role: 'Head of Company' }), current, runs, 'Anja'), null);
  assertEquals(sequenceEmailChange(parsed({ action: 'edit', contactKey: 'abrown', name: 'Mr A Brown', email: 'abrown@oak.sch.uk' }), mergeContacts(site, [])[1], runs, 'Anja'), null);
  assertEquals(sequenceEmailChange(parsed({ action: 'add', name: 'Mr J Khan', email: 'sbm@oak.sch.uk' }), null, runs, 'Anja'), null);
...
Deno.test('editorName and savedMessage', () => {
  assertEquals(editorName({ display_name: 'Anja Micic', email: 'anja@whofoundwho.co.uk' }, 'Anja Cold Targets'), 'Anja Micic');
  assertEquals(editorName({ display_name: null, email: 'anja@whofoundwho.co.uk' }, 'Anja Cold Targets'), 'Anja');
  assertEquals(editorName({ display_name: '', email: 'kim@whofoundwho.co.uk' }, 'Kim Webb'), 'Kim Webb');
  assertEquals(editorName({ display_name: null, email: '' }, null), 'a consultant');
  const change = { sequenceId: 'd1', from: 'a', to: 'b', note: '' };
  assertEquals(savedMessage(parsed({ action: 'edit', contactKey: 'hpatel', name: 'Mrs H Patel', email: 'hpatel@oak.sch.uk' }), change), 'Saved. Mrs H Patel now shows hpatel@oak.sch.uk. The follow-ups running with them will email the new address.');
