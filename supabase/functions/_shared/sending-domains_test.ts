...
Deno.test('parseSendingDomains falls back to the notify sub-domain', () => {
  assertEquals(parseSendingDomains(null), ['notify.bigfishrecruitment.co.uk']);
  assertEquals(parseSendingDomains('whofoundwho.co.uk'), ['notify.bigfishrecruitment.co.uk']);
  assertEquals(parseSendingDomains([]), ['notify.bigfishrecruitment.co.uk']);
  assertEquals(parseSendingDomains([42, 'not a domain']), ['notify.bigfishrecruitment.co.uk']);
  assertEquals(parseSendingDomains(['Notify.BigFishRecruitment.co.uk', 'whofoundwho.co.uk', 'whofoundwho.co.uk']), ['notify.bigfishrecruitment.co.uk', 'whofoundwho.co.uk']);
});

Deno.test('a consultant address on a verified domain is used as given', () => {
  const r = resolveFrom({ name: 'Anja Micic', email: 'Anja@whofoundwho.co.uk' }, ['notify.bigfishrecruitment.co.uk', 'whofoundwho.co.uk'], FALLBACK);
...
Deno.test('an address on an unverified domain keeps the name and falls back to the default sender', () => {
  const r = resolveFrom({ name: 'Anja Micic', email: 'anja@whofoundwho.co.uk' }, ['notify.bigfishrecruitment.co.uk'], FALLBACK);
  assertEquals(r.applied, false);
  assertEquals(r.from, 'Anja Micic <noreply@notify.bigfishrecruitment.co.uk>');
  assertEquals(r.domain, 'notify.bigfishrecruitment.co.uk');
...
Deno.test('Big Fish addresses are not on the list until Craig adds the domain', () => {
  const r = resolveFrom({ name: 'Craig Springett', email: 'craig@bigfishrecruitment.co.uk' }, ['notify.bigfishrecruitment.co.uk', 'whofoundwho.co.uk'], FALLBACK);
  assertEquals(r.applied, false);
  assertEquals(r.from, 'Craig Springett <noreply@notify.bigfishrecruitment.co.uk>');
});
